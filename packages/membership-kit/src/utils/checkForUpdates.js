...
const NPM_REGISTRY_URL = 'https://registry.npmjs.org/@cogbot/membership-kit/latest';
const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // once per day
const STORAGE_KEY = 'cogbot_kit_update_check';

/**
 * Checks npm for a newer version of @cogbot/membership-kit and logs
 * a console message if one is available. Only runs once per day and
 * only in development mode.
 *
 * Call this from your app's entry point:
 *   import { checkForUpdates } from '@cogbot/membership-kit';
 *   checkForUpdates();
 */
export async function checkForUpdates() {
  ...
  try {
    ...
    if (latestVersion && latestVersion !== installedVersion) {
      ...
      console.info(
        `%c@cogbot/membership-kit update available: ${installedVersion} → ${latestVersion} (${severity})%c\n` +
        `Run: npm update @cogbot/membership-kit`,
